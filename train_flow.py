...
from transform.build_transform import build_transform, build_flow_transform
...
def parse_option():
    ...
    parser.add_argument('--pretrained', type=str, default=None, help='path to pretrained unisiam model')
    parser.add_argument('--teacher_path', type=str, default=None, help='path to teacher model')
    parser.add_argument('--dataset', type=str, default='torFlow', choices=['torFlow'], help='dataset')
    ...
    parser.add_argument('--backbone', type=str, default='df', choices=['df', 'cnn'])
    parser.add_argument('--size', type=int, default=5000, help='input size')
    ...
    parser.add_argument('--n_query', type=int, default=20, help='n_query')
    parser.add_argument('--n_test_task', type=int, default=20, help='total test few-shot episodes')
    parser.add_argument('--test_batch_size', type=int, default=5, help='episode_batch_size')
...
def build_train_loader(args):
    train_transform = build_flow_transform(args)

    if args.dataset == 'torFlow':
        train_dataset = torFlowNet(
...
def build_fewshot_loader(args, mode='test'):
    ...
    test_transform = SimpleTrans(args.size)
...
def build_model(args):
    ...
    if args.backbone == 'df':
        encoder = df(in_dim=(1, args.size), emb_size=args.emb_size)
    elif args.backbone == 'cnn':
        encoder = cnn(in_dim=(1, args.size), emb_size=args.emb_size)
...
def main():
    ...
    # evaluate_fewshot(model.encoder, test_loader, n_way=args.n_way, n_shots=[5,20], n_query=args.n_query, classifier='LR', power_norm=True)
